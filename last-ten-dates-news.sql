...
-- Например, UNIX_TIMESTAMP(CURDATE())
...
SELECT CAST(FROM_UNIXTIME(`idate`) AS DATE) FROM `news` WHERE `idate` < UNIX_TIMESTAMP() GROUP BY CAST(FROM_UNIXTIME(`idate`) AS DATE) ORDER BY `idate` DESC LIMIT 10;
...
SELECT `idate` FROM `news` WHERE `idate` BETWEEN ADDDATE(NOW(), INTERVAL -30 DAY) AND NOW() GROUP BY `idate` ORDER BY `idate` DESC LIMIT 10;
